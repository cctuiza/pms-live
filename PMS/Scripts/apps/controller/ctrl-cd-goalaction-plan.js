...
app.controller('ctrlGoalAction', ['$scope', '$stateParams', '$state', '$http', '$rootScope', '$filter', 'filterFilter', '$AspCookie', 'HttpErrorService', 'GlobalHelper', 'Notification', '$localForage', '$timeout',
    function ($scope, $stateParams, $state, $http, $rootScope, $filter, filterFilter, $AspCookie, HttpErrorService, GlobalHelper, Notification, $localForage, $timeout) {
       ...
       //Date Show Year Only
        ...
        $scope.onInit = function () {
            ...
            //});
            $http.get(coreapi + 'GetCdpYearDropdown').success(function (data, status, headers, config) {
                $scope.cdpyear = data;
                $scope.year;
                for (var j = 0; j < $scope.cdpyear.length; j++) {

                    $scope.yearOptions.push({ value: $scope.cdpyear[j].drpdata, text: $scope.cdpyear[j].drpdisplay });
            ...
            if ($scope.action === "edit" || $scope.action === "view") { // Edit CDP
                $scope.isnew = false;
                $scope.isedit = true;
                ...
                if ($scope.type === "1") {
                    $scope.employeeId = $stateParams.em;
                    $http.get(coreapi + 'PersonalInfo?employeeId=' + $scope.employeeId).success(function (data, status, headers, config) {
                        $scope.personalInfo = data;
                        $scope.employename = $scope.personalInfo[0].employeename;
                    });
                    ...
                    $scope.showEmployeeNameDropdown = false;
                } else {
                    ...
                    //        $scope.employeeOptions.push({ value: $scope.subordinateInfo[i].employeeId, text: $scope.subordinateInfo[i].employeename });

                    //    }

                    //    $scope.showEmployeeName = false;
                    //    $scope.showEmployeeNameDropdown = true;
                    //});
                ...
                $http.get(coreapi + 'GetCDPperYear?employeeId=' + $scope.employeeId + '&cdpId=' + parseInt($scope.cdpId)).success(function (data, status, headers, config) {
                    $scope.year = data.yearno;
                });

              

                $scope.showYear = true;
                $scope.showYearDropdown = false;
            } else { //Add New CDP
