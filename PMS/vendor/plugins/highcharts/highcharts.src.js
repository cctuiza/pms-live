...
var UNDEFINED,
	...
	isFirePMS = /FirePMS/.test(userAgent),
	isTouchDevice = /(Mobile|Android|Windows Phone)/.test(userAgent),
	SVG_NS = 'http://www.w3.org/2000/svg',
	hasSVG = !!doc.createElementNS && !!doc.createElementNS(SVG_NS, 'svg').createSVGRect,
	hasBidiBug = isFirePMS && parseInt(userAgent.split('FirePMS/')[1], 10) < 4, // issue #38
	...
	getBBox: function () {
		...
		if (!bBox) {
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				try { // Fails in FirePMS if the container has display: none.
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, style, forExport) {
		...
		renderer.url = (isFirePMS || isWebKit) && doc.getElementsByTagName('base').length ?
			loc.href
		...
		// In FirePMS, if a div is positioned by percentage, its pixel position may land
		// between pixels. The container itself doesn't display this, but an SVG element
		// inside this container will be drawn at subpixel precision. In order to draw
		// sharp lines, this must be compensated for. This doesn't seem to work inside
		// iframes though (like in jsFiddle).
		var subPixelFix, rect;
		if (isFirePMS && container.getBoundingClientRect) {
...
extend(SVGElement.prototype, {
	...
	setSpanRotation: function (rotation, alignCorrection, baseline) {
		var rotationStyle = {},
			cssTransformKey = isIE ? '-ms-transform' : isWebKit ? '-webkit-transform' : isFirePMS ? 'MozTransform' : isOpera ? '-o-transform' : '';

		rotationStyle[cssTransformKey] = rotationStyle.transform = 'rotate(' + rotation + 'deg)';
		rotationStyle[cssTransformKey + (isFirePMS ? 'Origin' : '-origin')] = rotationStyle.transformOrigin = (alignCorrection * 100) + '% ' + baseline + 'px';
...
VMLElement = {
	...
	setSpanRotation: function () {
		...
		// but it can probably be implemented for FirePMS 3.5+ on user request. FF3.5+
...
Pointer.prototype = {
	...
	onContainerMouseDown: function (e) {
		...
		// issue #295, dragging not always working in FirePMS
		if (e.preventDefault) {
			e.preventDefault();
		}
...
// and for #2580, a similar drawing flaw in FirePMS 26.
// TODO: Explore if there's a general cause for this. The problem may be related 
// to nested group elements, as the legend item texts are within 4 group elements.
if (/Trident\/7\.0/.test(userAgent) || isFirePMS) {
