...
app.controller('ctrlPmsUtilizationD6', ['$scope', '$stateParams', '$state', '$http', '$rootScope', '$filter', 'filterFilter', '$AspCookie', 'HttpErrorService', 'GlobalHelper', 'Notification', '$localForage', '$timeout', '$window',
    function ($scope, $stateParams, $state, $http, $rootScope, $filter, filterFilter, $AspCookie, HttpErrorService, GlobalHelper, Notification, $localForage, $timeout, $window) {
        ...
        $scope.onInit = function () {
            ...
            $http.get(coreapi + 'Pms/pms-utilization-report-District6').success(function (response, data, status, headers, config) {
        ...
        $scope.ChangeYear = function () {
            ...
            $http.get(coreapi + 'Pms/pms-utilization-report-District6?year=' + $scope.yearcovered).success(function (response, data, status, headers, config) {
                $scope.pmsutilization = response;
