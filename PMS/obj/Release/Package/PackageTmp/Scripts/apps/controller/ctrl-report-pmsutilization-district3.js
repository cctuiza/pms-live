...
app.controller('ctrlPmsUtilizationD3', ['$scope', '$stateParams', '$state', '$http', '$rootScope', '$filter', 'filterFilter', '$AspCookie', 'HttpErrorService', 'GlobalHelper', 'Notification', '$localForage', '$timeout', '$window',
    function ($scope, $stateParams, $state, $http, $rootScope, $filter, filterFilter, $AspCookie, HttpErrorService, GlobalHelper, Notification, $localForage, $timeout, $window) {
        ...
        $scope.onInit = function () {
            ...
            $http.get(coreapi + 'Pms/pms-utilization-report-District3').success(function (response, data, status, headers, config) {
        ...
        $scope.ChangeYear = function () {
            ...
            $http.get(coreapi + 'Pms/pms-utilization-report-District3?year=' + $scope.yearcovered).success(function (response, data, status, headers, config) {
                $scope.pmsutilization = response;
