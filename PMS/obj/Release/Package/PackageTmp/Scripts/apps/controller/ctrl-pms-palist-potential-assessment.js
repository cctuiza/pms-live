app.controller('pms.palist.potential.assessment', ['$stateParams', '$state', '$http', '$rootScope', '$filter', '$AspCookie', '$modal', '$window', 'filterFilter', 'HttpErrorService', 'GlobalHelper',
    function ($stateParams, $state, $http, $rootScope, $filter, $AspCookie, $modal, $window, filterFilter, HttpErrorService, GlobalHelper) {
        ...
        self.navigateBehavioralAssessmentEntry = function (param) {
            $rootScope.closeModalForm();
            $state.go('employee-ba-entry', { hashcode: param });
        }
